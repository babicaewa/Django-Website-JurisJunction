...
$(document).ready(function() {
    ...
    $('#blogForm').submit(function (event) {
        ...
        if (!$('#topicAdded p').text()){
            recreateAlertMessage("You must add a Topic");
        } else if (!$('#provinceAdded p').text()) {
            recreateAlertMessage("You must add a Province");
        }

        else {
            $.ajax({
                type:'POST',
                url:'/user_blogs/edit_blog/',
                data: {
    ...
    function recreateAlertMessage(errorString) {
        // Check if the alert message div already exists
        if ($('#alertMessage').length) {
            $('#alertMessage').remove();
        }
        // Recreate the alert message div and append it to the body
        var alertMessage = '<div id="alertMessage" class="alert alert-danger alert-dismissible fade show fixed-alert alert-message sticky-top" role="alert">';
        ...
        var parentDiv = mainContentContainer.parent();

       $('.page-title').before(alertMessage);
    }
