$(document).ready(function() {
    ...
    var minRating = $("#minRating").data("min-rating");
...
$('.avg-rating-select').on('click', function(event) {
    ...
    var selectedRatingData = $(this).find('p').html();

    //console.log("whatt _ " + $(selectedRatingText).attr('class'))

    if ($(selectedRatingText).attr('class') == 'avg-rating-select-text any-star-rating') {
        var newDiv = $("<div>" + selectedRatingText + "</div>");
        newDiv.css('margin-top', '6px');
    } else {
    ...
    //console.log(selectedRatingData)
    switch (selectedRatingData) {
        case "5 Stars &amp; Up":
            //console.log('bvcvbv')
            $('#ratingSelected').val('5');
            break;
        case "4 Stars &amp; Up":
            $('#ratingSelected').val('4');
            break;
        case "3 Stars &amp; Up":
            $('#ratingSelected').val('3');
            break;
        case "2 Stars &amp; Up":
            $('#ratingSelected').val('2');
            break;
        case "1 Star &amp; Up":
            $('#ratingSelected').val('1');
            break;
...
function initPlaceholderUp(placeholderId) {
    placeholderId.css('margin-top', '-10px');
    placeholderId.css('font-size', '0.8rem');

}

if($('#languageInput').val() || $('#keywordInput').val() || $('#radiusInput').val() != '20' || minRating > 0) {
    $('.extra-filters').css('display', 'block');
}

if ($('#autocomplete').val()) {
    initPlaceholderUp(locationPlaceholder);
}
if ($('#specialtyInput').val()) {
    initPlaceholderUp(specialtyPlaceholder);
}
if ($('#languageInput').val()) {
    initPlaceholderUp(languagePlaceholder);
}
if ($('#keywordInput').val()) {
    initPlaceholderUp(keywordPlaceholder);
}

if (minRating > 0) {
    $('.rating-placeholder-text').css('margin-top', '-38px');
    $('.rating-placeholder-text').css('font-size', '0.8rem');
    ...
    switch(minRating) {
        case 5:
            starDivPrev = jQuery('#5star').clone();
            starDivText = $('<p>', {
                class: 'avg-rating-select-text',
                text: '5 Stars & Up'
            })
            var newDiv = $("<div>") 
            newDiv.append(starDivPrev, starDivText);
            newDiv.addClass("rating-selected-container");
            $('.rating-dropdown-inner-text').append(newDiv);
            $('rating-dropdown-inner-text').css('justify-content', 'flex-end');
            break;
        case 4:
            starDivPrev = jQuery('#4star').clone();
            starDivText = $('<p>', {
                class: 'avg-rating-select-text',
                text: '4 Stars & Up'
            })
            ...
            break;
        case 3:
            starDivPrev = jQuery('#3star').clone();
            starDivText = $('<p>', {
                class: 'avg-rating-select-text',
                text: '3 Stars & Up'
            })
            ...
            break;
        case 2:
            starDivPrev = jQuery('#2star').clone();
            starDivText = $('<p>', {
                class: 'avg-rating-select-text',
                text: '2 Stars & Up'
            })
            ...
            break;
        case 1:
            starDivPrev = jQuery('#1star').clone();
            starDivText = $('<p>', {
                class: 'avg-rating-select-text',
                text: '1 Star & Up'
            })
